...
public class QueryIdCachingProxyHandler extends ProxyHandler {
    ...
    public static final String CATALOG_HEADER = "X-Trino-Catalog";
    public static final String ALTERNATE_CATALOG_HEADER = "X-Presto-Catalog";
    ...
    private BlockingQueue<QueryDetails> queue;
    ...
    public QueryIdCachingProxyHandler(
            ...
            Meter requestMeter, BlockingQueue<QueryDetails> queue, ExternalPrestoConfiguration externalPresto) {
        ...
        this.queue = queue;
        this.externalPresto = externalPresto;
    }
    ...
    private boolean shouldRunOnExternalPresto(ExternalPrestoConfiguration externalPresto, String routingGroup) {
        if (externalPresto.isEnabled() && externalPresto.getRoutingGroup() == routingGroup) {
            return true;
        }
        return false;
    }
    ...
    public String rewriteTarget(HttpServletRequest request) {
        ...
        if (isPathWhiteListed(request.getRequestURI())) {
            ...
            if (!Strings.isNullOrEmpty(queryId)) {
                ...
                if (shouldRunOnExternalPresto(externalPresto, routingGroup)) {
                    try {
                        queue.put(getQueryDetail(request));
                    } catch (InterruptedException e) {
                        log.warn("Could not run query on external trino");
                    }
                }
    ...
    private QueryDetails getQueryDetail(HttpServletRequest request) {
        String requestBody = null;
        try {
            requestBody = CharStreams.toString(request.getReader());
        } catch (IOException e) {
            log.warn("Not able to read query request to run with external instance");
        }

        String user = Optional.ofNullable(request.getHeader(USER_HEADER))
                .orElse(request.getHeader(ALTERNATE_USER_HEADER));
        String source = Optional.ofNullable(request.getHeader(SOURCE_HEADER))
                .orElse(request.getHeader(ALTERNATE_SOURCE_HEADER));
        String catalog = Optional.ofNullable(request.getHeader(CATALOG_HEADER))
                .orElse(request.getHeader(ALTERNATE_CATALOG_HEADER));
        QueryDetails queryDetails = new QueryDetails(requestBody, user, source, catalog);
        return queryDetails;
    }
